    ...
    <div className="group relative flex min-h-[380px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg md:min-h-[438px]">
      ...
      <div className="flex min-h-[230px] flex-col gap-3 p-5 md:gap-4">
        {!hidePrice && (
          <div className="flex gap-2">
            <span className="p-semibold-14 w-min rounded-full bg-green-100 px-4 py-1 text-green-700">
              {event.isFree ? strings.EventDetails.isFree : `$${event.price}`}
            </span>
        ...
        <div className="flex-between w-full">
          ...
          {hasOrderLink && (
            <Link href={`/orders?eventId=${event._id.toString()}`} className="flex gap-2">
              <p className="text-primary-500">
                {strings.EventCard.orderDetailsTitle}
              </p>
              <Image
